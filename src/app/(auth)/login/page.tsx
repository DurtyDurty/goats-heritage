...
function LoginForm() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (!result.data.session) {
      setError("No session created. Please verify your email first.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="w-full max-w-md rounded-xl border border-[#262626] bg-[#141414] p-8">
      ...
      <form onSubmit={handleLogin} className="mt-8 space-y-4">
        ...
        <div>
          <label className="mb-1.5 block text-sm text-[#A3A3A3]">Password</label>
          <input
            type="password"
            required
            value={password}
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-3 rounded-lg bg-white px-4 py-3 font-medium text-black transition-colors hover:bg-gray-100"
      >
      ...
      <p className="mt-6 text-center text-sm text-[#A3A3A3]">
        ...
        <Link href="/signup" className="text-[#C8A84E] transition-colors hover:text-[#E8D48B]">
          Sign Up
        </Link>
      </p>
    </div>
...
export default function LoginPage() {
  return (
    <div className="flex min-h-[80vh] items-center justify-center px-4">
      <Suspense fallback={<div className="h-96 w-full max-w-md animate-pulse rounded-xl bg-[#141414]" />}>
        <LoginForm />
      </Suspense>
    </div>
  );
